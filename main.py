...
def main():
	...
	parser.add_argument('--dataset', dest='dataset', type=str, default='test_dataset', help='Main folder of the dataset')
	...
	for xfp in xFilepaths:
		...
		if img:
			...
			# Split img into 2*2 blocks (image size: 52 * 52)
			#shaped = blockshaped(lbp_value, 26, 26)

			# Split img into 4*4 blocks (image size: 52 * 52)
			#shaped = blockshaped(lbp_value, 13, 13)

			# Split img into 13*13 blocks (image size: 52 * 52)
			shaped = blockshaped(lbp_value, 4, 4)
